...
# There are a lot of bugs and dead code; I apologize.
# I'll try to refator it later.
...
from dl_source.memory_set import MemorySet
...
def find_board_name(board_url, logger=None):
    """
...
def find_high_res_links(body):
    ...
    # parent = low_res_imgs.find_parent(attrs={"data-test-id": "more-ideas-container"})

    # FIXME
    return [img["src"] for img in low_res_imgs if img.find_parent(attrs={"data-test-id": "masonry-container"})], len(low_res_imgs)


def retrieve_bord_info(board_url, download_folder, body, num_pins=None, board_name=None, logger=None):
    """
    ...
    if board_name is None:
        board_name = find_board_name(board_url, logger=logger)

    # Find the number of pins to download, minimum between available pins and requested pins
    num_available_pins = find_num_pins(body, logger=logger)
    if num_pins is None:
...
def _handle_download_report(future, url, logger):
    """
    ...
    :param logger: Logger used for the download report.
...
class Downloader(object):
    ...
    def __init__(self, download_folder, size_verifier, logger=None):
        """
        ...
        :param logger: Logger to use for download output.
        """
    ...
    def download_high_res(self, high_res_source):
        ...
        if title in self.previously_downloaded:
            ...
            try:
                ...
                self.logger.warning(f"Connection died during download of Pin {title}.")
                status_report["downloaded"] = False
...
class PinterestDownloader(object):
    ...
    def __init__(self, page_timeout=10, num_threads=4,
                 min_resolution="0x0", size_compare_mode=None, logger=None):
        """
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        if os.path.isfile("geckodriver.log"):
            self.logger.debug("Removing geckodriver.log")
    ...
    def download_board(self, board_url, download_folder,
        ...
        board_name, num_pins, download_folder = retrieve_bord_info(board_name=board_name,
                                                                   ...
                                                                   body=body,
                                                                   logger=self.logger)
        self.logger.info(f"Found board '{board_name}' with {num_pins} pins")

        num_skipped = 0
        downloaded_this_time = 0
        url_cache = MemorySet()
        downloader = Downloader(download_folder, self.size_verifier, logger=self.logger)
        ...
        with concurrent.futures.ThreadPoolExecutor(self.num_threads) as consumers:
            ...
            while downloaded_this_time < num_pins and num_skipped < skip_tolerance:
                ...
                for fut in concurrent.futures.as_completed(future_to_url):
                    ...
                    succesfully_downloaded = _handle_download_report(future=fut, url=url, logger=self.logger)
                ...
                if num_srcs < num_pins:
                    self.logger.debug(f"Need to scroll down because {num_srcs} < {num_pins}")
                    body = self.scroll_down_for_new_body(times=7)

        if num_skipped >= skip_tolerance:
            self.logger.debug("Skip limit reached. Stopping.")
        self.logger.info(
            f"""Downloaded {downloaded_this_time} pins to {download_folder}. Skipped {num_skipped} pins. Finished.""")
        
    # Not working correctly.
    # It captures ~30 image links instead of ~42.
    # Needs to be fixed in the future.
    # FIXME
    def handle_image_links(self, board_url):
        # related-interests-multi-column-module
        imgs = []
        ...
        def collect_current():
            ...
            low_res_imgs = soup.find_all("img")
            imgs.extend(
                img["src"] for img in low_res_imgs 
                    if not img.find_parent(attrs={"data-test-id": "more-ideas-container"})
                    and not img.find_parent(attrs={"data-test-id": "related-interests-multi-column-module"})
            )
        ...
        for _ in range(7):
            self.scroll_down_for_new_body(times=1)
            collect_current()
            time.sleep(3)
    ...
    # FIXME
    # Since not all the photos download on the first try, I think I'll try fetching them again
    def download_by_image_links(self, links: list[str], download_folder: str) -> None:
        downloader = Downloader(download_folder, self.size_verifier, logger=self.logger)
        for link in links:
            downloader.download_high_res(link)

    def download_board_v2(self, board_url, download_folder):
        img_list = self.handle_image_links(board_url)
        self.download_by_image_links(img_list, download_folder)
